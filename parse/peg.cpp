...
void peg_t::load(grammar_t &grammar)
{
	...
	grammar.rules.push_back(rule_t(PEG, "peg::peg", true));
	DEFINITION = grammar.rules.size();
	grammar.rules.push_back(rule_t(DEFINITION, "peg::definition", true));
	IMPORT = grammar.rules.size();
	grammar.rules.push_back(rule_t(IMPORT, "peg::import", true));
	CHOICE = grammar.rules.size();
	grammar.rules.push_back(rule_t(CHOICE, "peg::choice", true));
	SEQUENCE = grammar.rules.size();
	grammar.rules.push_back(rule_t(SEQUENCE, "peg::sequence", true));
	TERM = grammar.rules.size();
	grammar.rules.push_back(rule_t(TERM, "peg::term", true));
	NAME = grammar.rules.size();
	grammar.rules.push_back(rule_t(NAME, "peg::name", true));
	...
	n[30] = grammar.insert(new keyword(";", true));
	...
	n[34] = grammar.insert(new keyword("=", true));
	n[35] = grammar.insert(new keyword("@=", true));
	n[36] = grammar.insert(new whitespace(true, false));
	n[37] = grammar.insert(new instance(true));
	n[38] = grammar.insert(new character("\\0", true));
	n[39] = grammar.insert(new whitespace(true, false));
	n[40] = grammar.insert(new stem(2, true));
	...
	n[39]->next.push_back(n[41]);
	n[39]->next.push_back(n[40]);
	n[39]->next.push_back(n[38]);
	n[40]->next.push_back(n[39]);
	n[41]->next.push_back(n[39]);
	n[42]->next.push_back(n[41]);
	n[42]->next.push_back(n[40]);
	n[42]->next.push_back(n[38]);

	grammar.rules[0].start.push_back(n[42]);
	grammar.rules[1].start.push_back(n[37]);
